...
3. **Middleware System** (`src/services/middleware/`)
   - 10 built-in middleware types for message processing
...
**Worker Process Exit Issues:**
If you encounter "A worker process has failed to exit gracefully and has been force exited. This is likely caused by tests leaking", this indicates:

- **Resource Leaks**: Tests are not properly cleaning up resources (timers, connections, file handles)
- **Async Operations**: Unresolved promises or ongoing async operations preventing clean exit
- **Event Listeners**: Unclosed event listeners or subscriptions
- **External Connections**: Open database connections, HTTP clients, or WebSocket connections

**Common Solutions:**
- Add proper cleanup in `afterEach` or `afterAll` hooks
- Mock external dependencies that create persistent connections
- Clear timers and intervals in test cleanup
- Close file handles and streams
- Ensure all async operations complete or are properly cancelled
...
4. **Rebuild TypeScript**: Compile TypeScript to JavaScript (REQUIRED for CLI changes)
   ```bash
   bun run build
   ```

5. **Fix ALL Issues**: **MUST** achieve 0 failures and 0 errors
   - Fix any test failures
   - Fix any linting errors (warnings are acceptable, but errors are NOT)
   - Fix any TypeScript compilation errors
   - Re-run tests, lint, and build until everything passes
