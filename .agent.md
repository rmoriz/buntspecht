...
### Test Commands**:
  ```bash
  bun run test           # Run all tests (Jest via Node.js - PRIMARY)
...
- **Primary Test Suite**: Still uses Jest via `bun run test` (which calls npm test)
...
2. **Run All Tests**: Ensure nothing is broken
   ...
   bun run test
   ```
   Note: This runs the primary Jest test suite via npm/Node.js
